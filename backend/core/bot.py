        ...
        try:
                ...
                if change <= -config.stop_loss:
                    ...
                    if drop <= -config.trailing_stop_pct:
                        ...
                        state.last_signal_reason = (
                            f"Trailing stop triggered ({drop*100:.1f}% from peak ₩{state.peak_price:,.0f})"
                        )
                    else:
            ...
            if signal_action == "buy" and state.position == "none":
                ...
                if not allowed:
                    db = await get_persistent_db()
                    await write_audit_log(
                        db, "risk_blocked", reason,
                        market=config.market, mode=config.mode
                    )
                    state.last_signal_reason = f"Risk blocked: {reason}"
                    logger.info("Buy blocked by risk engine [%s]: %s", config.market, reason)
                else:
                    if config.mode == "paper":
            ...
            # ── Sell ─────────────────────────────────────────────
                ...
                if config.mode == "paper":
                    pnl = await _paper_sell(config, state, price)
                else:
                    pnl = await _live_sell(config, state, price)

                if pnl is not None:
                    risk.on_trade_result(config.mode, pnl)
                    # consecutive_losses 동기화
                    state.consecutive_losses = risk.get_state(config.mode).consecutive_losses
                    # circuit breaker 발동 시 봇 자동 중지
                    risk_status = risk.get_status(config.mode)
                    if risk_status["circuit_breaker_triggered"]:
                        db = await get_persistent_db()
                        await write_audit_log(
                            db, "circuit_breaker",
                            "Circuit breaker triggered, stopping bot",
                            market=config.market, mode=config.mode,
                            details={"pnl": pnl, "daily_pnl": risk_status["daily_pnl"]}
                        )
                        logger.warning(
                            "Circuit breaker triggered [%s] — stopping bot", config.market
                        )
                        state.running = False
                        break

            # ── positions 테이블에 상태 저장 ─────────────────────
            if config.mode == "paper":
                db = await get_persistent_db()
                await upsert_position(
                    db, "paper", config.market,
                    krw_balance=state.paper_krw,
                    asset_balance=state.paper_asset,
                    entry_price=state.entry_price,
                    position=state.position,
